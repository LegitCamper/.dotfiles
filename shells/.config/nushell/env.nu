...
let-env PROMPT_MULTILINE_INDICATOR = (^/home/sawyer/.cargo/bin/starship prompt --continuation)
...
let-env PROMPT_COMMAND = { ||
    ...
    ^/home/sawyer/.cargo/bin/starship prompt $"--cmd-duration=($env.CMD_DURATION_MS)" $"--status=($env.LAST_EXIT_CODE)" $"--terminal-width=($width)"
}
...
let-env PROMPT_COMMAND_RIGHT = { ||
    let width = (term size).columns
    ^/home/sawyer/.cargo/bin/starship prompt --right $"--cmd-duration=($env.CMD_DURATION_MS)" $"--status=($env.LAST_EXIT_CODE)" $"--terminal-width=($width)"
}